                            ...
                            <div className={"container desktop-comments h-auto"}>
                                <div className={"col-md-12 row"}>
                                    <div className={"col-4"}>
                                        ...
                                        <p className={"text-white p-pas"}>Отзыв: Пребывание в гостинице ModernShosh было приятным опытом. Очень впечатлен уровнем обслуживания и профессионализмом персонала. Внимательное отношение к деталям и быстрое реагирование на запросы сделали мой отдых комфортным и беззаботным.</p>
                                    </div>
                                    <div className={"col-4 "}>
                                        <h5 className={"text-white text-center lato pt-4"}>Lyudmila Safaeva :</h5>
                                        <p className={"text-center lato text-white"}>{localStorage.getItem("lang") === "uzb" ? "Qoniqarli mijoz" : localStorage.getItem("lang") === "rus" ? "Довольный клиент" : "Satisfied customer"}</p>
                                        <p className={"text-white p-pas"}>Отзыв: Очень доволен услугами гостиницы ModernShosh. Вежливый и отзывчивый персонал всегда готов помочь. Номера чистые и удобные, уборка проводилась своевременно. Завтраки в ресторане были вкусными и разнообразными.</p>
                                    </div>
                                    <div className={"col-4 "}>
                                        <h5 className={"text-white text-center lato pt-4 "}>Murad Ivanov :</h5>
                                        <p className={"text-center lato text-white "}>{localStorage.getItem("lang") === "uzb" ? "Qoniqarli mijoz" : localStorage.getItem("lang") === "rus" ? "Довольный клиент" : "Satisfied customer"}</p>
                                        <p className={"text-white p-pas"}>Отзыв: Хочу выразить благодарность за отличное обслуживание в гостинице ModernShosh. Все мои просьбы и вопросы решались оперативно. Уровень сервиса на высоте, персонал профессиональный и внимательный.</p>
